...
class CartManager {
    ...
    normalizeStoredCart(storedCart) {
        if (Array.isArray(storedCart)) {
            return {
                items: storedCart.map(item => this.normalizeCartItem(item)).filter(Boolean)
            };
        }

        if (storedCart && Array.isArray(storedCart.items)) {
            return {
                items: storedCart.items.map(item => this.normalizeCartItem(item)).filter(Boolean)
            };
        }
    ...
    normalizeCartItem(item) {
        ...
        if (item.product && item.product.id != null) {
            return {
                ...
                priceAtTime: this.getNumericPrice(item.priceAtTime ?? item.product.price)
            };
        }
    ...
    getStoredCart(username) {
        const localCart = this.normalizeStoredCart(JSON.parse(localStorage.getItem('localCart')));
        if (localCart.items.length > 0 || !username) {
            return localCart;
        }

        const legacyCart = this.normalizeStoredCart(JSON.parse(localStorage.getItem(`cart_${username}`)));
        if (legacyCart.items.length > 0) {
            localStorage.setItem('localCart', JSON.stringify(legacyCart));
        }
        return legacyCart;
    }
    ...
    async loadCart() {
        try {
            ...
            const localCart = this.getStoredCart(currentUser?.username);
            this.cartItems = localCart.items;
            return localCart;
